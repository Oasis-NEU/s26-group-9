...
export default function SummaryPanel({ detailMode = 'overview', activeDetail = {}, tasks = [], sessions = [] }) {
        ...
        return (
            <section className="summary-panel">
                ...
                <article className="summary-panel__focus-card">
                    ...
                    {focusTask ? (
                        <>
                            <p className="summary-panel__focus-name">{focusTask.title || focusTask.name || 'Untitled Task'}</p>
                    ...
                    {dueSoon.length === 0 ? (
                        ...
                        <ul className="summary-panel__list summary-panel__list--plain">
                            {dueSoon.map((task) => {
                                ...
                                return (
                                    <li key={task?.id || `${task?.title || task?.name}-${task?.due_date}`} className="summary-panel__deadline-item">
                                        <span className="summary-panel__deadline-name">{task?.title || task?.name || 'Untitled Task'}</span>
    ...
    if (detailMode === 'task') {
        ...
        return (
            <section className="summary-panel">
                <h1 className="summary-panel__title">{task?.title || task?.name || 'Task'} - summary</h1>
        ...
        <section className="summary-panel">
            ...
            <article className="summary-panel__focus-card">
                ...
                <p className="summary-panel__focus-name">
                    {friendActiveTask ? friendActiveTask?.title || friendActiveTask?.name : 'Currently idle'}
                </p>
            </article>
